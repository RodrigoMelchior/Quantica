...
import br.com.web.pesquisas.domain.specification.ContatoSpecification;
...
import br.com.web.pesquisas.web.rest.dto.FiltroContatoDTO;
...
public class ContatoResource extends EntityServiceBasedRestController<Contato, Long, ContatoService>{
    ...
    public ResponseEntity<List<Contato>> pesquisar(@RequestBody FiltroContatoDTO filtro) {
        ...
        Conjunction<Contato> specification = makeSpecification(filtro);
        Page<Contato> pageResult = null;
        
        pageResult = this.service.findBySpecification(specification, new PageRequest(filtro.getPage() - 1, filtro.getLimit()));
        
        return new ResponseEntity<List<Contato>>(pageResult.getContent(),
                HeadersUtil.generatePaginationHttpHeaders(pageResult.getTotalElements()), HttpStatus.OK);   
    }


    
    private Conjunction<Contato> makeSpecification(FiltroContatoDTO filtro) {
        List<Specification<Contato>> specifications = new ArrayList<Specification<Contato>>();
        
        if (StringUtils.isNotEmpty(filtro.getEmpresa())) {
            specifications.add(ContatoSpecification.igualEmpresa(filtro.getEmpresa()));
        }
        return new Conjunction<Contato>(specifications);
    }
