...
public class ArquivoServiceImpl extends CrudServiceImpl<Arquivo, Long, ArquivoRepository> implements ArquivoService{
	...
	private void salvarDadosEmpresa(String[] arrLinha, Arquivo arquivo) {
		...
		BigDecimal inpc = inpcService.calculaIndiceReajuste(100L, arquivo.getPesquisa().getId());
		Integer posicao = 8;
		...
		for (ItemPesquisa item : itens) {
	    	...
	    	registroItem.setVlrValor(new BigDecimal(arrLinha[posicao].toString()).multiply(inpc).toString());
	...
	@Override
	public Arquivo findByUsuarioAndPesquisa(Long idUser, Long idPesquisa) {
		Usuario usuario = usuarioService.findById(idUser);
		Pesquisa pesquisa = pesquisaService.findById(idPesquisa);
		return repository.findByUsuarioAndPesquisa(usuario, pesquisa);
	}
